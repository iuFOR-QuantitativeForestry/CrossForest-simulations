...
#SBATCH -o Input.Generator.sim009-%j.o
#SBATCH -e Input.Generator.sim009-%j.e
...
## make input files - the number as input or 90000 whether no input
...
if [ $# -eq 5 ]; then           nt=$3;  np=$2; nf=$1;    nf1=$4;  nf2=$5
          ...
          else                  nt=100; np=1;  nf=90000; nf1=300; nf2=300
fi
...
OUT_Dir=/scratch/uva_iufor_1/uva_iufor_1_3/sim009
...
##i=1
##while [ $i -le $nf ]; do
##	filename=scnr_sp999_${i}_1-100.json
##	sed -e "s/_idnumber_/${i}/g" ~/simanfor/scripts/sim009.template.json > ${filename}
