...
#SBATCH -q normal
# nombre
#SBATCH -J RScrpt.Plot.Generator.sim001
# tiempo maximo de ejecucion (p.e. 2 dias). Maximo permitido: 5 dias
#SBATCH --time=01:00:00
# archivos de salida y de error
#SBATCH -o RScrpt.Plot.Generator.sim001-%j.o
#SBATCH -e RScrpt.Plot.Generator.sim001-%j.e
...
SCR_Dir=~/scripts
OUT_Dir=/scratch/uva_iufor_1/uva_iufor_1_3/sim001/input
...
if [ $# -eq 3 ]; then       nt=$3; np=$2; nf=$1
  elif [ $# -eq 2 ]; then   nt=20; np=$2; nf=$1
    elif [ $# -eq 1 ]; then nt=20; np=2;  nf=$1 
      else                  nt=20; np=2;  nf=2
fi
...
i=1
while [ $i -le $nf ]; do
    ~/scripts/sim000.PlotsGenerator.EN.hpc.R id.file=$i sp=999 n.p=$np n.t=$nt age0=25 ht.m=12 dbh.m=11 tph=1111 var.d=4 var.h=2
    ((i++))    
done
cd $CurrentDir
