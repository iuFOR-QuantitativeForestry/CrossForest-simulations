...
if len(sys.argv)>3:
    ...
    if len(sys.argv)>2:
        ...
        if len(sys.argv)>1:
            ...
            file_name = 'ppt.3p.xlsx'
            file_type = 'csv'
        else:
            scratch_folder = 'real001'
            file_name = 'ppt.3p.xlsx'
            file_type = 'csv'

## Directorio de trabajo
Path=scratch_folder+'/input/complete/' # '/scratch/uva_iufor_1/uva_iufor_1_3/'
print(Path)

## step 001 Read input excel to dataframe "plot" and "tree"
fileinpath = Path+file_name
...
for plot_name in plotlist:
    ...
    fileoutpath = fileinpath.replace('.xlsx', '.P'+str(i)+'.xlsx', 1)
